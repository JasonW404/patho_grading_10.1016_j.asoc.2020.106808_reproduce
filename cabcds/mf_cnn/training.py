...
def train_mf_cnn_modules(
    ...
    seg_model.to(device)
    seg_optimizer = optim.SGD(seg_model.parameters(), lr=0.001, momentum=0.9)
    seg_criterion = nn.CrossEntropyLoss() # Standard for pixel-wise classification
    
    seg_model.train()
    for epoch in range(10): # Epochs not specified, early stopping was used [cite: 217]
    ...
    det_model.to(device)
    det_optimizer = optim.SGD(det_model.parameters(), lr=0.001, momentum=0.9)
